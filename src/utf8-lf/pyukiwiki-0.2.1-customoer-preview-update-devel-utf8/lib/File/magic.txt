...
# animation formats
# MPEG, FLI, DL originally from <vax (at) ccwf (dot) cc (dot) utexas (dot) edu> (VaX#n8)
# FLC, SGI, Apple originally from Daniel Quinlan (<quinlan (at) yggdrasil (dot) com>)
...
# Jan Nicolai Langfeldt (<janl (at) ifi (dot) uio (dot) no>), Dan Quinlan (<quinlan (at) yggdrasil (dot) com>),
# and others
...
# various lisp types, from Daniel Quinlan (<quinlan (at) yggdrasil (dot) com>)
0	string	;;
...
# Daniel Quinlan, <quinlan (at) yggdrasil (dot) com>
...
# From: Florian La Roche <florian (at) knorke (dot) saar (dot) de>
...
# Keith Waclena <keith (at) cerberus (dot) uchicago (dot) edu>
# Send additions to <perl5-porters (at) perl (dot) org>
...
# From: David Necas <yeti (at) physics (dot) muni (dot) cz>
...
# AVI section extended by Patrik R�dman <patrik+file-magic (at) iki (dot) fi>
...
>8	string	sfbk	SoundFont/Bank
#----------------------------------------------------------------------
# RPM: file(1) magic for Red Hat Packages   Erik Troan (<ewt (at) redhat (dot) com>)
0	beshort	0xedab
...
# Duncan P. Simpson, <D (dot) P (dot) Simpson (at) dcs (dot) warwick (dot) ac (dot) uk>
...
# from Daniel Quinlan (<quinlan (at) yggdrasil (dot) com>)
# adapted to string extenstions by Anthon van der Neut <<anthon (at) mnt (dot) org>)
