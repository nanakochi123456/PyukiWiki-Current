...
#
# 投稿内容のメール送信時のprefix
$mailform::subject_prefix="$::mail_head$::mail_head{form}"
	...
	if($errstr eq '' && $::form{edit} eq '') {
		if($::form{confirm} ne '') {
			...
			if($::write_location eq 0) {
				...
				if($::write_location eq 1) {
					&location("$::basehref?cmd=mailform&sent=true&refer=@{[&encode($::form{refer})]}", 302, $::HTTP_HEADER);
					close(STDOUT);
					exit;
				}
			}
...
=item PyukiWiki/Plugin/Standard/mailform

L<http://pyukiwiki.info/PyukiWiki/Plugin/Standard/mailform/>

=item PyukiWiki CVS

L<http://sfjp.jp/cvs/view/pyukiwiki/PyukiWiki-Devel/plugin/mailform.inc.pl?view=log>

L<http://sfjp.jp/cvs/view/pyukiwiki/PyukiWiki-Devel-UTF8/plugin/mailform.inc.pl?view=log>

L<http://cvs.pyukiwiki.info/cgi-bin/cvsweb.cgi/PyukiWiki-Devel/plugin/mailform.inc.pl?view=log>

L<http://cvs.pyukiwiki.info/cgi-bin/cvsweb.cgi/PyukiWiki-Devel-UTF8/plugin/mailform.inc.pl?view=log>
